...
pos = nx.spring_layout(ws_graph, seed=42)  # 시각화를 위한 레이아웃
nx.draw(
    ws_graph, pos, with_labels=True, labels=nx.get_node_attributes(ws_graph, 'name'),
...
for node in ws_graph.nodes():
    total_weight = sum(ws_graph[node][neighbor]['weight'] / 2 for neighbor in ws_graph.neighbors(node))
...
adjusted_node_weights = {node: node_weights[node] * pagerank[node] for node in ws_graph.nodes()}
