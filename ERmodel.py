...
pos = nx.spring_layout(er_graph, k=10.0, iterations=5)  # 시각화를 위한 레이아웃
nx.draw(
    er_graph,
    pos,
    with_labels=True,
    labels=nx.get_node_attributes(er_graph, "name"),
    node_size=1500,
...
for node in er_graph.nodes():
    total_weight = sum(
        er_graph[node][neighbor]["weight"] / 2 for neighbor in er_graph.neighbors(node)
    )
...
adjusted_node_weights = {
    node: node_weights[node] * pagerank[node] for node in er_graph.nodes()
}
